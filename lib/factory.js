const sha1 = async i => Array.from(new Uint8Array(await crypto.subtle.digest('SHA-1', (new TextEncoder()).encode(i))), byte => byte.toString(16).padStart(2, '0')).join(''),
    rx = {
        ...
        const $this = this, parseX = async (xCode, entry, name) => {
            ...
            while (aStructMatches.length || aUnionMatches.length) {
                for (const m of aStructMatches) {
                    const [identifier, map] = buildStructFromMatch(m), structName = await sha1(JSON.stringify(Array.from(map.entries())))
                    structs[structName] ??= map
                    xCode = cleanXCode(m[0], `\n${structName} ${identifier};\n`)
                }
                for (const m of aUnionMatches) {
                    const [identifier, discriminant, arms] = buildUnionFromMatch(m), unionEntry = { discriminant, arms },
                        unionName = await sha1(JSON.stringify(unionEntry))
                    unions[unionName] ??= unionEntry
        ...
        const typeClass = await parseX(xCode, entry, name)
